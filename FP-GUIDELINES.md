...
check was introduced: **137 findings across 13 files
and 4,469 lines**. CI compares against that budget and fails only when a
...
|---|---:|---|---|---|
| `RS003` | 80 | error | typed errors | panic-based control flow (`unwrap`/`expect`/`panic!`) |
| `RS001` | 36 | warn | immutable values | mutable local binding (`let mut`) |
| `RS005` | 5 | warn | typed errors | untyped/erased error in a signature |
| `RS004` | 4 | warn | illegal states excluded by types | wildcard match arm defeats exhaustiveness |
| `RS008` | 4 | warn | explicit outputs | `&mut self` method returning unit |
| `XX001` | 4 | warn | composition | oversized module |
| `RS002` | 3 | error | explicit state transitions | module-level mutable / shared-interior global |
...
*typed errors* · 80 occurrences at baseline
...
*immutable values* · 36 occurrences at baseline
...
*explicit state transitions* · 3 occurrences at baseline
