...
pub fn load_from(lookup: impl Fn(&str) -> Option<String>) -> CliEnvValues {
    CliEnvValues {
        ...
        log_filter: lookup("RUST_LOG").filter(|value| !value.is_empty()).unwrap_or_else(|| "info,hyper=warn".to_string()),
        otel_exporter_otlp_endpoint: lookup("OTEL_EXPORTER_OTLP_ENDPOINT").filter(|value| !value.is_empty()),
        otel_resource_attributes: lookup("OTEL_RESOURCE_ATTRIBUTES").filter(|value| !value.is_empty()),
    }
}
...
pub fn load_env_map(
    ...
    let log_filter = pick(&["RUST_LOG"], &["flags", "env_shell", "env_file"], shell, dotenv, flags, Some("info,hyper=warn"));
    ...
    let otel_resource_attributes = pick(&["OTEL_RESOURCE_ATTRIBUTES"], &["flags", "env_shell", "env_file"], shell, dotenv, flags, None);
    if let Some(value) = otel_resource_attributes {
        out.insert("OTEL_RESOURCE_ATTRIBUTES".to_string(), value);
    }
    Ok(out)
}
